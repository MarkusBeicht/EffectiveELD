...
for domainid in eukdomains.keys():
  ...
  while p > 0.05:
    ...
    P,p = wilcoxon(differences, alternative='less')
  

  #small effect size: r > 0.1
  threshold_01, p, r = 0, 1, 0
  while p > 0.05 or r < 0.1:
    threshold_01 += 1
    ...
    #the positive ranks (Q) are calculated using the overall ranks (Pmax) and the negative ranks (P) from the wilcoxon test
    n = len(differences)
    Pmax = (n * (n + 1)) / 2
    Q = Pmax - P
    r = abs(P - Q) / Pmax
  ...
  threshold_03, p, r = 0, 1, 0
  while p > 0.05 or r < 0.3:
    threshold_03 += 1
    #the differences between the thershold value that is being tested and the genomecounts are calculated (ties are set = +0.5, in support of the null-hypothesis)
  ...
  threshold_05, p, r = 0, 1, 0
  while p > 0.05 or r < 0.5:
    threshold_05 += 1
    #the differences between the thershold value that is being tested and the genomecounts are calculated (ties are set = +0.5, in support of the null-hypothesis)
  ...
  #very large effect size: r > 0.7
  threshold_07, p, r = 0, 1, 0
  while p > 0.05 or r < 0.7:
    threshold_07 += 1
    #the differences between the thershold value that is being tested and the genomecounts are calculated (ties are set = +0.5, in support of the null-hypothesis)
