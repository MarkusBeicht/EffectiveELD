...
"""
    #writes each significantly enriched domain with their highest reached threshold (p = 0.05, 0.01, 0.00001) into "euk_score"-file
    (t05, t01, t00001) = thresholds[domainid]
    if count >= t00001:
      poutfile.write("%s\t%s\t%1.5f\n" % (domainid, refseq_accession, 0.00001))
    elif count >= t01:
      poutfile.write("%s\t%s\t%1.5f\n" % (domainid, refseq_accession, 0.01))
    elif count >= t05:
      poutfile.write("%s\t%s\t%1.5f\n" % (domainid, refseq_accession, 0.05))
"""
